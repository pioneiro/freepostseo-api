...
const pushurl = (newURL, callback) => {
	try {
		urldb.findOne({ url: newURL }, (errorFinding, foundURL) => {
			if (errorFinding)
				callback({
					error: { message: errorFinding.message || "Error accessing DB" },
				});
				...
				new urldb({ url: newURL })
					...
					.catch((errorSaving) =>
						callback({
							error: { message: errorSaving.message || "Error accessing DB" },
						})
					);
		});
	} catch (error) {
	...
	try {
		urldb.findById(urlid, (errorFinding, foundURL) => {
			if (errorFinding) {
				...
				callback({ error: { message: "No URL registered for given ID" } });
			}
		});
	} catch (error) {
		callback({ error: { message: error.message || "Unexpected Error" } });
	}
};
